...
class MF(nn.Cell):
    ...
    def construct(self, IterCount_PSO, trainData, trainCount, validData, validCount, ppso):
        ...
        for t in range(1, IterCount_PSO + 1):
            ...
            for p in range(ppso):
                for i in range(R):
                    aid = int(trainData[i][0])
                    bid = int(trainData[i][1])
                    valueHat = getPrediction(aid, bid, self.S, self.D, self.a, self.b)
                    err = trainData[i][2] - valueHat
                    for r in range(1, rank + 1):
                ...
                # 评估适应度函数
                for i in range(Rv):
                    aid = int(validData[i][0])
                    bid = int(validData[i][1])
                    valueHat = getPrediction(aid, bid, self.S, self.D, self.a, self.b)
                    fitness[p] += (validData[i][2] - valueHat) ** 2
...
def evolution(S, D, a, b, testData, testCount):
    ...
    MAE = 0
    for i in range(R):
        aid = int(testData[i][0])
        bid = int(testData[i][1])
        valueHat = getPrediction(aid, bid, S, D, a, b)
        RSME += (testData[i][2] - valueHat) ** 2
        MAE += abs(testData[i][2] - valueHat)
...
def Calculate_a(arr, u, lamb):
    ...
    for trainTuple in train_data:
        if trainTuple[0] == u:
            count += 1
            temp += (trainTuple[2] - getPrediction(int(trainTuple[0]), int(trainTuple[1]), S, D, a, b)) ** 2
...
if __name__ == '__main__':
    ...
    # "C:\\Users\\Lx\\Desktop"
    # "D:\\PythonWorkplace\\Matrix\\1M\\0.7-0.1-0.2"

    # 加载训练、验证和测试数据
    train_data = Tensor(
        np.loadtxt("D:\\PythonWorkplace\\Matrix\\10K\\0.7-0.1-0.2\\ML_train_0.7.txt", delimiter="::", dtype=float),
        dtype=mindspore.float32)
    valid_data = Tensor(
        np.loadtxt("D:\\PythonWorkplace\\Matrix\\10K\\0.7-0.1-0.2\\ML_val_0.1.txt", delimiter="::", dtype=float),
        dtype=mindspore.float32)
    test_data = Tensor(
        np.loadtxt("D:\\PythonWorkplace\\Matrix\\10K\\0.7-0.1-0.2\\ML_test_0.2.txt", delimiter="::", dtype=float),
        dtype=mindspore.float32)
    ...
    print("test minRMSE: ", RMSE)
    print("test minMAE: ", MAE)